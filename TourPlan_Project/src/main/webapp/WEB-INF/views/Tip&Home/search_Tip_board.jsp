...
<form action="search_Tip_board" method="post">
	<div align="center" >
	<h1 class="text-stroke" style="text-align: center; color: white; font-family:Broadway;">${subject} : ${word} Search Result</h1>
	...
	<table style="width: 650px; border-radius: 5px; font-size: 9pt;">
		...
		<c:choose>
			...
			<tr style="background-color: white; border-radius: 5px;">
				<td colspan="6" style="text-align: center; font-weight: bold;">검색결과가 존재하지 않습니다.</td>
			</tr>
			</c:when>
			<c:otherwise>
		...
		<tr align="center" style="background-color: #0064CD; border-radius: 5px;">
			<td>  
				<select id="indexOfSearchTipBoard" name="indexOfSearchTipBoard">
				...
				<c:forEach begin="1" end="${totalPage}" step="1" var="cnt">
					<c:choose>
						<c:when test="${cnt eq start}">
							<a href="search_Tip_board?start=${cnt}&subject=${subject}&word=${word}"style="color: black; font-weight:bold; font-size:10pt;">[${cnt}]</a>
						</c:when>
						<c:otherwise>
							<a href="search_Tip_board?start=${cnt}&subject=${subject}&word=${word}"style="color: black; text-decoration: none;">[${cnt}]</a>
						</c:otherwise>
					</c:choose>
