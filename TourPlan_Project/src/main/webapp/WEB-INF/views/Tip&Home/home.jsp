...
<div align="center">
	...
	<div align="center">
		<table border="0" style="border-spacing: 10px;">
			...
			<tr style="height: 150px;">
				<td rowspan="2" style="border-radius: 5px; width: 500px;">
					<c:import url="best_Travle_board.jsp"/>
				</td>
			</tr>
			<tr style="height: 150px;">
			<% 
...
<script>//채팅 관련 스크립트
	...
	$(document).ready(function(){
		$.ajax({
			url:'${pageContext.request.contextPath}/getHistoryFirst',
			dataType: 'jsonp',
			...
			success: function(input){
				...
				for(let i=0;i<(input.chat).length;i++){
					...
					if((input.chat)[i].userId == input.userId){						
						 var name = $('<div align=left style="font-weight:bold; text-align:cneter;"></div>').append("나 (" + (input.chat)[i].userNickname + ")");
						 var date = $('<font style="text-align:cneter;" align=center></font>').append((input.chat)[i].str_time);
						...
						var name = $('<div align=right style="font-weight:bold; text-align:cneter;"></div>').append((input.chat)[i].userNickname+"<br>");
						 var date = $('<font style="text-align:cneter" align=center></font>').append((input.chat)[i].str_time);
	...
	function SendChat(){
		...
		if(value == null || value==''){//입력칸이 비었을 경우
			...
			$.ajax({
				url:'${pageContext.request.contextPath}/sendTheChat',
				data: params,
				dataType: 'jsonp',
