...
async defer></script>
	...
	<div id="drag-container">
		...
		<div id="content_right">
			<textarea id="plan_content"  readonly="readonly" name="content"></textarea>
	...
	<div id="div_cal" class="cal">
		...
		<div id="div_cal_button" class="cal">
			<button type="button" id="div_cal_ok" class="cal" >확인</button>
		</div>
	</div>
	<!-- 내정보 메뉴 bar -->
	<ul id="userMenu">
	...
	<script>
		...
		function callAppendChild(data){
			...
			var button = document.createElement('button');    // button 엘리먼트 생성
			button.setAttribute("id","btn"+schedule);
			var text = document.createTextNode(schedule); // text 엘리먼트 생성
			button.appendChild(text);   // button 엘리먼트에 text를 자식으로 추가
			li.appendChild(button);        // li 엘리먼트에 button를 자식으로 추가
			mainmenu.appendChild(li);      // target 이라는 ID를 가진 ul 태그의 맨 마지막 자식으로 추가
			//버튼으로 일정번호 추가시 그에 맞게 마지막에 일정 날짜 자동으로 추가.
			*/
