...
<form action="search_QA_board" method="post">
	<div align="center">
		<h1  class="text-stroke" style="font-weight:bold; text-align: center; color: white; font-family:verdana; font-size:50pt;" >Q & A </h1>
		<div style="width:700px; padding-bottom:5px; padding-top:50px;" align="right">
			<input class="mainButton" type="button" value="목록으로" style="width: 75px;" onclick="location.href='QA_board'">
			<input class="mainButton" type="button" value="글 작성" style="width: 60px; " onclick="location.href='Write_QA_board'">
		</div>
		...
		<!-- 이전 버튼 -->
			...
			<td colspan="6" align="center">
				<c:choose>
					<c:when test="${start > 1 }">
						<button class="mainButton" type="button" onclick="location.href='QA_board?start=${start-1}'
				...
				<c:forEach begin="1" end="${totalPage}" step="1" var="cnt">
					<c:choose>
						<c:when test="${cnt eq start}">
							<a href="QA_board?start=${cnt}"style="color: black; font-size:10pt;">[${cnt}]</a>
						</c:when>
						<c:otherwise>
							<a href="QA_board?start=${cnt}"style="color: black; text-decoration: none;">[${cnt}]</a>
						</c:otherwise>
					</c:choose>
