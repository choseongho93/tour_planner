			...
			<td colspan="6" align="center">
				<c:choose>
					...
					<!-- 페이지가 1보다 크면 이전 버튼 클릭 시 start-1로 start 보냄 /  -->
						<button class="mainButton" type="button" onclick="location.href='search_Suggestion_board?start=${start-1}&subject=${subject}&word=${word}'">이전</button>
					</c:when>
					<c:otherwise>
			...
			<!-- totalPage(총 게시글 수)가 보여지고 싶은 게시글 수(term)보다 크면 totalPage(총 페이지 수)는 나눈 몫+1로 지정-->
				...
				<c:forEach begin="1" end="${totalPage}" step="1" var="cnt">
					<c:choose>
						<c:when test="${cnt eq start}">
							<a href="search_Suggestion_board?start=${cnt}&subject=${subject}&word=${word}"style="color: black; font-size:10pt;">[${cnt}]</a>
						</c:when>
						<c:otherwise>
							<a href="search_Suggestion_board?start=${cnt}&subject=${subject}&word=${word}"style="color: black; text-decoration: none;">[${cnt}]</a>
						</c:otherwise>
					</c:choose>
				...
				<c:choose>
					...
					<!-- 위에 지정한 totalpage(총 페이지 수)보다 start가 작으면 start+1로 start보냄 -->
						<button class="mainButton" type="button" onclick="location.href='search_Suggestion_board?start=${start+1}&subject=${subject}&word=${word}'">다음</button>	
					</c:when>
					<c:otherwise>
